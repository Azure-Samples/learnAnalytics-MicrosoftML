...
model <- rxLogisticRegression(Label ~ Pixels, 
                              data = train_df,
...
Let's create another dataset for distinguishing between individuals in the dataset.

```{r faces}


faces <- data.frame(
  path = list.files("../data/faces", recursive = TRUE, full.names = TRUE),
  stringsAsFactors = FALSE
) %>% tbl_df

faces$label <- str_split(faces$path, "/") %>% map_chr(c(4))


sample_len <- floor(0.5 * nrow(faces))
set.seed(123)
train_indices <- sample(seq_len(nrow(faces)), size = sample_len)

train_df <- faces[train_indices, ]
test_df <- faces[-train_indices, ]
```


Now we can featurize and train our model:

```{r featurize and train}
...
preds <- rxPredict(model, test_df, extraVarsToWrite = "label")
preds <- preds %>% mutate(label = factor(label),
                          PredictedLabel = factor(label))
rxCube( ~ label : PredictedLabel , data = preds,
       returnDataFrame = TRUE) -> pred_conf
